...
func main() {
	...
	v1 := r.Group("/api")
	{
		...
		protected := v1.Group("/v1").Use(middlewares.Authz())
		{
			...
			protected.GET("userconnection/:id", api.GetUsersForConnectionTable)
			...
			protected.GET("post/user/:id", api.GetPostsByUserId)
			protected.GET("post/post/:id", api.GetPostsByPostId)
